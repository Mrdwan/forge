...
def _fail_aider(
    error: str = "aider failed", files: list[str] | None = None
) -> AiderResult:
...
class TestRunPreCommit:
    ...
    def test_one_command_fails(self, tmp_path: Path) -> None:
        with patch("src.pipeline.subprocess.run") as mock_run:
            mock_run.side_effect = [
                _make_proc(1, "lint error", "more details"),
                _make_proc(0),
            ]
    ...
    def test_error_message_truncated_at_1500(self, tmp_path: Path) -> None:
        long_output = "e" * 2000
        with patch(
            "src.pipeline.subprocess.run", return_value=_make_proc(1, long_output, "")
        ):
...
class TestFinalizeStep:
    def test_happy_path(
        self, forge_config: ForgeConfig, memory_path: Path, sample_step: Step
    ) -> None:
        result = StepResult(
            status=StepStatus.SUCCESS,
            step=sample_step,
            summary="ok",
            details="senior review",
        )
        with (
    ...
    def test_skips_when_not_success(
        self, forge_config: ForgeConfig, sample_step: Step
    ) -> None:
    ...
    def test_commit_failure_stops_early(
        self, forge_config: ForgeConfig, sample_step: Step
    ) -> None:
        result = StepResult(
            status=StepStatus.SUCCESS, step=sample_step, summary="ok", details="d"
        )
        with (
